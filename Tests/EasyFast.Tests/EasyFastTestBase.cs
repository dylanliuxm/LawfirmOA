...
namespace EasyFast.Tests
{
    public abstract class EasyFastTestBase : AbpIntegratedTestBase<EasyFastTestModule>
    {
        ...
        protected void UsingDbContext(Action<EasyFastDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EasyFastDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EasyFastDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EasyFastDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EasyFastDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EasyFastDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EasyFastDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EasyFastDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EasyFastDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EasyFastDbContext, Task<T>> func)
        {
            T result;
